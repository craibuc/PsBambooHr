...
# Get-BambooHrUser.ps1
$sut = (Split-Path -Leaf $MyInvocation.MyCommand.Path) -replace '\.Tests\.', '.'

# . /PsBambooHr/PsBambooHr/Public/Get-BambooHrUser.ps1
$Path = Join-Path $PublicPath $sut
Write-Debug "Path: $Path"

. (Join-Path $PublicPath $sut)

Describe "Get-BambooHrUser" -Tag 'unit' {

    Context "Parameter validation" {

        BeforeAll {
            $Command = Get-Command 'Get-BambooHrUser'
        } 
    ...
    Context "Usage" {
        ...
        BeforeEach {
            # act
            Get-BambooHrUser -ApiKey $ApiKey -Subdomain $Subdomain
        }
        ...
        Context "Request" {
            ...
            It "uses the correct Uri" {
                ...
                Assert-MockCalled Invoke-WebRequest -ParameterFilter { 
                    # Write-Debug "Uri: $Uri"
                    $Uri -eq "https://api.bamboohr.com/api/gateway.php/$Subdomain/v1/meta/users/" 
                }
            }
